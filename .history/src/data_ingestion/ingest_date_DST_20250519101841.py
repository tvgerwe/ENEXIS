...
def create_datetime_rows(start_date, end_date):
    ...
    local_tz = pytz.timezone("Europe/Amsterdam") # Set the local timezone to Europe/Amsterdam

# Convert UTC datetime to local time
df["datetime_local"] = df["datetime"].dt.tz_convert(local_tz)  
df["is_DST"] = df["datetime_local"].apply(lambda x: bool(x.dst()))
# Convert back to UTC
df["datetime"] = df["datetime_local"].dt.tz_convert("UTC")
