...
def get_connection(db_path):
    """Return a sqlite3 connection, maak de folder aan indien nodig."""
...
def ensure_tables_exist(conn_data, conn_log):
    """Maak de ingest-log tabel aan indien deze nog niet bestaat."""
...
def get_last_timestamp(conn):
    """Return de maximum waarde van TIMESTAMP_COLUMN uit raw_ned_obs of None als table ontbreekt."""
...
def create_table_from_df(conn, df, table_name):
    """Maak een nieuwe tabel met kolommen gebaseerd op DataFrame."""
...
def remove_duplicates(conn, table_name):
    """Verwijder exacte duplicaten uit de tabel, behoud de eerste rij per unieke set."""
    cur = conn.cursor()
...
def write_log(conn, start_time, end_time, api_endpoint, rows_fetched, last_timestamp, status, error_message):
    """Schrijf een entry naar de ingestie-log."""
    conn.execute(
        """
...
def main():
    ...
    try:
        ...
        if all_records:
            ...
            last_timestamp = prev_ts or DEFAULT_START_DATE
