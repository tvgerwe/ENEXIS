...
HORIZON = 168
FORECAST_LAG_HOURS = 36 # adding defintion for forecast lag
FORECAST_HORIZON_HOURS = 144 # specifying the forecast horizon
...
def build_training_set(train_start, train_end, run_date):
    ...
    run_date = pd.Timestamp(run_date, tz="UTC")run_date = pd.Timestamp(run_date, tz="UTC").replace(hour=12, minute=0, second=0)
    forecast_start = run_date + pd.Timedelta(hours=FORECAST_LAG_HOURS) # Adjusting for forecast lag
    forecast_end = forecast_start + pd.Timedelta(hours=FORECAST_HORIZON_HOURS - 1) # Adjusting for forecast horizon
