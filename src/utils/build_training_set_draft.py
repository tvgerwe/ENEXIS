...
TOTAL_FORECAST_HOURS = FORECAST_LAG_HOURS + FORECAST_HORIZON_HOURS
...
def build_training_set(train_start, train_end, run_date):
    ...
    forecast_end = run_date + pd.Timedelta(hours=TOTAL_FORECAST_HOURS - 1)
    ...
    try:
        ...
        logger.info(f"🧐 Forecast rows: {df_preds.shape}")
        logger.info(f"🧐 target_datetime min: {df_preds['target_datetime'].min()} | max: {df_preds['target_datetime'].max()}")
        ...
        logger.info(f"🎯 Geselecteerde forecast rijen: {df_preds.shape[0]}")
        ...
        # === Add actual prices (for forecast range only)
